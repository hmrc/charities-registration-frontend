...
class CharitiesConnector @Inject()(httpClient: HttpClient, implicit val appConfig: FrontendAppConfig) {
  ...
  def getUserAnswers(id: String)(
    implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Option[UserAnswers]] = {
    ...
    httpClient.GET[HttpResponse](charitiesRegistrationUrl)(implicitly, hc, ec) map { response =>
      response.status match {
        case OK => response.json.validate[UserAnswers] match {
          case JsSuccess(validResponse, _) => Some(validResponse)
          case JsError(errors) => logger.error(s"[CharitiesConnector][getUserAnswers]: Unexpected response, $errors returned")
            throw JsResultException(errors)
        }
        case _ => logger.warn(s"[CharitiesConnector][getUserAnswers]: no data found for user $id")
          None
      }
    }
  }

  def saveUserAnswers(userAnswers: UserAnswers)(
    implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Boolean] = {

    val charitiesRegistrationUrl: String = s"${appConfig.getCharitiesBackend}/charities-registration/saveUserAnswer/${userAnswers.id}"

    httpClient.POST[JsValue, HttpResponse](charitiesRegistrationUrl, Json.toJson(userAnswers))(implicitly, implicitly, hc, ec) map {

      case HttpResponse(OK, responseBody, _) => Json.parse(responseBody).validate[SaveStatus] match {
        case JsSuccess(value, _) => value.status
        case JsError(errors) => throw JsResultException(errors)
      }

      case error => logger.error(s"[CharitiesConnector][saveUserAnswers]: Unexpected response returned " + error)
