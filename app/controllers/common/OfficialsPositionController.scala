...
trait OfficialsPositionController extends LocalBaseController {
  ...
  def getView(page: QuestionPage[OfficialsPosition], form: Form[OfficialsPosition], fullName: String, submitCall: Call)(
    implicit
  ...
  def postView(
    mode: Mode,
    page: QuestionPage[OfficialsPosition],
    form: Form[OfficialsPosition],
    fullName: String,
    ...
    form
      .bindFromRequest()
      .fold(
        formWithErrors => Future.successful(BadRequest(view(formWithErrors, fullName, messagePrefix, submitCall))),
        value =>
          for {
