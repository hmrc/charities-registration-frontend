...
class AmendOtherOfficialsAddressController @Inject()(
  ...
  override val messagePrefix: String = "amendOtherOfficialAddress"
  ...
  def onPageLoad(mode: Mode, index: Index): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      getFullName(OtherOfficialsNamePage(index)) { otherOfficialsName =>
        ...
        Future.successful(getView(OtherOfficialAddressLookupPage(index),
          controllers.otherOfficials.routes.AmendOtherOfficialsAddressController.onSubmit(mode, index),
          countryService.countries(), Some(otherOfficialsName)))
      }
  }
  ...
  def onSubmit(mode: Mode, index: Index): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      getFullName(OtherOfficialsNamePage(index)) { otherOfficialsName =>
        ...
        postView(mode, OtherOfficialAddressLookupPage(index), Section8Page,
          controllers.otherOfficials.routes.AmendOtherOfficialsAddressController.onSubmit(mode, index),
          countryService.countries(), Some(otherOfficialsName))
      }
  }
