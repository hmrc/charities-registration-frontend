...
class WhyNotRegisteredWithCharityController @Inject()(
   ...
   formProvider: WhyNotRegisteredWithCharityFormProvider,
   val controllerComponents: MessagesControllerComponents,
   view: WhyNotRegisteredWithCharityView
  )(implicit appConfig: FrontendAppConfig) extends LocalBaseController {
  val form = formProvider()

  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>

    val preparedForm = request.userAnswers.get(WhyNotRegisteredWithCharityPage) match {
      case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    ...
    form.bindFromRequest().fold(
      ...
      value =>
        for {
          updatedAnswers <- Future.fromTry(request.userAnswers.set(WhyNotRegisteredWithCharityPage, value).flatMap(_.set(Section2Page, false)))
