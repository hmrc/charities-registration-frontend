    ...
    Form(
      mapping(
        "line1"    -> text(s"$messagePrefix.addressLine1.error.required")
          ...
          .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.addressLine1.error.format")),
        "line2"    -> optional(
          text()
            .verifying(maxLength(maxLength, s"$messagePrefix.addressLine2.error.length"))
            .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.addressLine2.error.format"))
        ),
        "line3"    -> optional(
          text()
            .verifying(maxLength(maxLength, s"$messagePrefix.addressLine3.error.length"))
            .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.addressLine3.error.format"))
        ),
        "town"     -> text(s"$messagePrefix.townOrCity.error.required")
          .verifying(maxLength(maxLength, s"$messagePrefix.townOrCity.error.length"))
          .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.townOrCity.error.format")),
