    ...
    Form(
      mapping(
        ...
        "firstName"  -> text(s"$messagePrefix.firstName.error.required")
          ...
          .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.firstName.error.format")),
        "middleName" -> optional(
          text()
            .verifying(maxLength(maxLength, s"$messagePrefix.middleName.error.length"))
            .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.middleName.error.format"))
        ),
        "lastName"   -> text(s"$messagePrefix.lastName.error.required")
          .verifying(maxLength(maxLength, s"$messagePrefix.lastName.error.length"))
          .verifying(regexp(validateFieldIncludingForeignCharacters, s"$messagePrefix.lastName.error.format"))
