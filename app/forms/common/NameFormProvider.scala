    ...
    Form(
      mapping(
        ...
        "firstName"  -> text(s"$messagePrefix.firstName.error.required")
          ...
          .verifying(regexpInclForeignExclLigatures( s"$messagePrefix.firstName.error.format")),
        "middleName" -> optional(
          text()
            .verifying(maxLength(maxLength, s"$messagePrefix.middleName.error.length"))
            .verifying(regexpInclForeignExclLigatures( s"$messagePrefix.middleName.error.format"))
        ),
        "lastName"   -> text(s"$messagePrefix.lastName.error.required")
          .verifying(maxLength(maxLength, s"$messagePrefix.lastName.error.length"))
          .verifying(regexpInclForeignExclLigatures( s"$messagePrefix.lastName.error.format"))
