...
class RegulatorsAndDocumentsNavigator extends BaseNavigator {
    ...
    case CharityOtherRegulatorDetailsPage =>
      (userAnswers: UserAnswers) =>
        userAnswers.get(CharityRegulatorPage) match {
          case Some(items) =>
            nextNav(
              CharityRegulator.pageMap.filter { case (key, _) => items.contains(key) }.values.toSeq,
              userAnswers,
              CheckMode
            )
  ...
  private def isCharityRegulatorPageNav(mode: Mode, userAnswers: UserAnswers): Call =
  ...
  private def isCharityRegulatorCheckNav(mode: Mode, userAnswers: UserAnswers): Call =
    (
      userAnswers.get(IsCharityRegulatorPage),
      userAnswers.get(CharityRegulatorPage),
      userAnswers.get(SelectWhyNoRegulatorPage)
    ) match {
      ...
      case (None, _, _)           => routes.PageNotFoundController.onPageLoad()
      case _                      => regulatorDocsRoutes.RegulatorsSummaryController.onPageLoad()
    }

  private def selectWhyNoRegulatorPageNav(userAnswers: UserAnswers, mode: Mode): Call =
    userAnswers.get(SelectWhyNoRegulatorPage) match {
  ...
  private def switchOverNavigation(remainingPages: Seq[QuestionPage[?]]): Call =
    remainingPages match {
      ...
      case ScottishRegulatorRegNumberPage :: _          =>
      ...
      case CharityOtherRegulatorDetailsPage :: _        =>
