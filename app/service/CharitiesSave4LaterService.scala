...
class CharitiesSave4LaterService @Inject() (
  ...
  private def isSection8Completed(userAnswers: UserAnswers): Try[UserAnswers] =
    userAnswers.get(OtherOfficialsNamePage(0)) match {
      case Some(_) =>
        userAnswers.set(
          Section8Page,
          OtherOfficialStatusHelper.checkComplete(userAnswers) && OtherOfficialStatusHelper.validateDataFromOldService(
            userAnswers
          )
        )
  ...
  private def updateSwitchOverUserAnswer(userAnswers: UserAnswers, result: TransformerKeeper)(implicit
    ...
    if (userAnswers.data.fields.nonEmpty) {
      Future
        .fromTry(result =
          isSection1Completed(userAnswers)
  ...
  private def checkForValidApplicationJourney(request: OptionalDataRequest[_], lastSessionId: Option[String])(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext,
    rh: RequestHeader
  ): Future[Either[Call, UserAnswers]] =
    (request.userAnswers, lastSessionId) match {
      case (Some(userAnswers), _)  =>
        Future
          .fromTry(result =
            isSection7Completed(userAnswers)
              .flatMap(userAnswers => isSection8Completed(userAnswers))
          )
      ...
      case _                       =>
        logger.error(s"[CharitiesSave4LaterService][getCacheData] no eligibility data and current session data found")
        Future.successful(Left(controllers.routes.CannotFindApplicationController.onPageLoad))
    }
