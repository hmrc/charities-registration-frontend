...
class UserAnswerTransformer extends JsonTransformer {
  ...
  private def ninoOrPassport(
    uaPathNino: JsPath,
    uaPathPassport: JsPath,
    uaFlagPath: JsPath,
  ...
  private def authorisedOfficialOriginalKey(index: Int, authOrOther: String): JsPath =
    __ \ s"${authOrOther}OfficialIndividual${index + 1}"
