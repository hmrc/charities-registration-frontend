...
class CharityContactDetailsCheckYourAnswersHelper() extends ImplicitDateFormatter
  with SummaryListRowHelper with CurrencyFormatter {

  def rows(charityContactDetails: CharityContactDetails)( implicit messages: Messages): Seq[SummaryListRow] = Seq(
    ...
    Some(
      summaryListRow(
        ...
        controllers.charityInformation.routes.CharityContactDetailsController.onPageLoad(CheckMode) -> messages("site.edit")
      )
    ),

    charityContactDetails.mobilePhone.map( phone =>
      summaryListRow(
