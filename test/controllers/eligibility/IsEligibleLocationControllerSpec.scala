...
import forms.eligibility.IsEligibleLocationFormProvider
...
import pages.eligibility.IsEligibleLocationPage
import play.api.inject.bind
import play.api.inject.guice.GuiceApplicationBuilder
import play.api.test.Helpers._
import repositories.SessionRepositoryImpl
import views.html.eligibility.IsEligibleLocationView
...
class IsEligibleLocationControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  val view: IsEligibleLocationView = injector.instanceOf[IsEligibleLocationView]
  val formProvider: IsEligibleLocationFormProvider = injector.instanceOf[IsEligibleLocationFormProvider]
  val form = formProvider()

  val controller: IsEligibleLocationController = inject[IsEligibleLocationController]
  ...
  "IsEligibleLocation Controller" must {
    ...
    "return OK and the correct view for a GET when user has already answered" in {
      ...
      when(mockSessionRepository.get(any())).thenReturn(Future.successful(Some(
        emptyUserAnswers.set(IsEligibleLocationPage, true).getOrElse(emptyUserAnswers))))
