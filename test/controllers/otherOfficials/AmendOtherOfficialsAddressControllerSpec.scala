...
class AmendOtherOfficialsAddressControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private val messageKeyPrefix = "amendOtherOfficialAddress"
  ...
  private val controller: AmendOtherOfficialsAddressController = inject[AmendOtherOfficialsAddressController]

  private val requestArgs = Seq("line1" -> "23", "line2" -> "Morrison street", "line3" -> "",
    "town" -> "Glasgow",
    "postcode" -> "G58AN",
    "country" -> "GB")
  private val localUserAnswers: UserAnswers = emptyUserAnswers.set(OtherOfficialAddressLookupPage(0),
    AddressModel(Seq("7", "Morrison street near riverview gardens", "Glasgow"), Some("G58AN"), CountryModel("GB", "United Kingdom")))
    .flatMap(_.set(OtherOfficialsNamePage(0), Name(SelectTitle.Mr, "Jim", Some("John"), "Jones"))).success.value


  "AmendOtherOfficialsAddressController Controller " must {

    "return OK and the correct view for a GET" in {

      val amendOtherOfficialsAddress = AmendAddressModel("7", Some("Morrison street near riverview gardens"), Some(""), "Glasgow", "G58AN", "GB")
      ...
      contentAsString(result) mustEqual view(form.fill(amendOtherOfficialsAddress), messageKeyPrefix,
        controllers.otherOfficials.routes.AmendOtherOfficialsAddressController.onSubmit(NormalMode, Index(0)),
