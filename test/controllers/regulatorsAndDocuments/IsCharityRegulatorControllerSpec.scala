...
class IsCharityRegulatorControllerSpec extends SpecBase with BeforeAndAfterEach {
  ...
  private val view: IsCharityRegulatorView                 = inject[IsCharityRegulatorView]
  ...
  "IsCharityRegulator Controller " must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      when(mockCharitiesConnector.getUserAnswers(any())(any(), any())).thenReturn(
        Future.successful(Right(Some(emptyUserAnswers.set(IsCharityRegulatorPage, true).getOrElse(emptyUserAnswers))))
      )
