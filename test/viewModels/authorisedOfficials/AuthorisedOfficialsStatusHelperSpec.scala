...
class AuthorisedOfficialsStatusHelperSpec extends SpecBase {
  ...
  def common(index: Int, userAnswers: UserAnswers): UserAnswers = userAnswers
    ...
    _.set(AuthorisedOfficialsPositionPage(index), OfficialsPosition.values.head)).flatMap(
    _.set(AuthorisedOfficialAddressLookupPage(index), ConfirmedAddressConstants.address)).success.value
  ...
  "AuthorisedOfficialsStatusHelper" must {
    ...
    "when verifying section 7 answers" must {
      ...
      "return true when one authorised official with Nino and no previous address (Scenario 2)" in {
        helper.checkComplete(commonData(0, emptyUserAnswers).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(AuthorisedOfficialsNinoPage(0), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, false)).success.value) mustBe true
      }

      "return false when one authorised official with no required data for Nino and no previous address (Scenario 2)" in {
        helper.checkComplete(commonData(0, emptyUserAnswers).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, false)).success.value) mustBe false
      }
      ...
      "return true when two authorised official with Nino and no previous address (Scenario 2)" in {
        helper.checkComplete(commonData(1, commonData(0, emptyUserAnswers)).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(AuthorisedOfficialsNinoPage(0), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAuthorisedOfficialNinoPage(1), true)).flatMap(
          _.set(AuthorisedOfficialsNinoPage(1), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(1), false)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, true)).success.value) mustBe true
      }
      ...
      "return true when two authorised official with Nino, passport and no previous address (Scenario 2)" in {
        helper.checkComplete(commonData(1, commonData(0, emptyUserAnswers)).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(AuthorisedOfficialsNinoPage(0), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAuthorisedOfficialNinoPage(1), false)).flatMap(
          _.set(AuthorisedOfficialsPassportPage(1), Passport("GB12345", "GB", LocalDate.of(year, month, dayOfMonth)))).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(1), false)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, true)).success.value) mustBe true
      }
      ...
      "return false when two authorised official with no required data for Nino, passport and no previous address (Scenario 2)" in {
        helper.checkComplete(commonData(1, commonData(0, emptyUserAnswers)).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          ...
          _.set(IsAuthorisedOfficialNinoPage(1), false)).flatMap(
          _.set(AuthorisedOfficialsPassportPage(1), Passport("GB12345", "GB", LocalDate.of(year, month, dayOfMonth)))).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, true)).success.value) mustBe false
      }

      "return true when one authorised official with Nino and previous address (Scenario 3)" in {
        helper.checkComplete(commonData(0, emptyUserAnswers).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(AuthorisedOfficialsNinoPage(0), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), true)).flatMap(
          _.set(AuthorisedOfficialPreviousAddressLookupPage(0), ConfirmedAddressConstants.address)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, false)).success.value) mustBe true
      }
      ...
      "return true when two authorised official with Nino and previous address (Scenario 3)" in {
        helper.checkComplete(commonData(1, commonData(0, emptyUserAnswers)).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          ...
          _.set(IsAuthorisedOfficialNinoPage(1), true)).flatMap(
          _.set(AuthorisedOfficialsNinoPage(1), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(1), true)).flatMap(
          _.set(AuthorisedOfficialPreviousAddressLookupPage(1), ConfirmedAddressConstants.address)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, true)).success.value) mustBe true
      }
      ...
      "return true when one authorised official with passport and no previous address (Scenario 4)" in {
        helper.checkComplete(commonData(0, emptyUserAnswers).set(IsAuthorisedOfficialNinoPage(0), false).flatMap(
          _.set(AuthorisedOfficialsPassportPage(0), Passport("GB12345", "GB", LocalDate.of(year, month, dayOfMonth)))).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, false)).success.value) mustBe true
      }
      ...
      "return false when two authorised official with no required data for passport, nino and no previous address (Scenario 4)" in {
        helper.checkComplete(commonData(1, commonData(0, emptyUserAnswers)).set(IsAuthorisedOfficialNinoPage(0), false).flatMap(
          _.set(AuthorisedOfficialsNinoPage(0), "AA123456A")).flatMap(
          _.set(IsAuthorisedOfficialNinoPage(1), true)).flatMap(
          _.set(AuthorisedOfficialsPassportPage(0), Passport("GB12345", "GB", LocalDate.of(year, month, dayOfMonth)))).flatMap(
          _.set(IsAddAnotherAuthorisedOfficialPage, true)).success.value) mustBe false
      ...
      "return false when not correct data for one authorised official" in {
        helper.checkComplete(commonData(0, emptyUserAnswers).set(IsAuthorisedOfficialNinoPage(0), true).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).flatMap(
          _.set(IsAuthorisedOfficialPreviousAddressPage(0), false)).success.value) mustBe false
      }
