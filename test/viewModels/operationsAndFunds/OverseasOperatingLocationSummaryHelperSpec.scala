...
class OverseasOperatingLocationSummaryHelperSpec extends SpecBase with SummaryListRowHelper with CurrencyFormatter {
  ...
  (_.set(WhatCountryDoesTheCharityOperateInPage(0) ,"TH").flatMap
  ...
  (_.set(WhatCountryDoesTheCharityOperateInPage(4) ,"AF"))).flatMap
  ...
  "Overseas Operating Location Summary Helper" must {
    ...
    "overseas Operating Location Summary CYA Row" must {
      ...
      "have a correctly formatted summary list row" in {
        val countries = s"""<div>Thai</div><div>India</div><div>Portugal</div><div>Paraguay</div><div>Afghanistan</div>"""
