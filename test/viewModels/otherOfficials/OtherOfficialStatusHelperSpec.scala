...
class OtherOfficialStatusHelperSpec extends SpecBase {
  ...
  implicit class OtherStatusPageSetter(userAnswers: UserAnswers) {
    ...
    def addAddress(index: Int): UserAnswers =
      userAnswers.set(IsOtherOfficialsPreviousAddressPage(index), true)
        .flatMap(_.set(OtherOfficialPreviousAddressLookupPage(index), ConfirmedAddressConstants.address)).success.value

    def addPassport(index: Int): UserAnswers =
      userAnswers.set(IsOtherOfficialNinoPage(index), false)
        .flatMap(_.set(OtherOfficialsPassportPage(index), Passport("GB12345", "GB", LocalDate.of(year, month, dayOfMonth)))).success.value
  ...
  def common(index: Int, userAnswers: UserAnswers): UserAnswers = userAnswers
    .set(OtherOfficialsNamePage(index), Name(SelectTitle.Mr, firstName = "John", None, lastName = "Jones")).flatMap(
    _.set(OtherOfficialsDOBPage(index), LocalDate.of(year, month, dayOfMonth))).flatMap(
    _.set(OtherOfficialsPhoneNumberPage(index), PhoneNumber(daytimePhone = "07700 900 982",
      mobilePhone = Some("07700 900 982")))).flatMap(
    _.set(OtherOfficialsPositionPage(index), OfficialsPosition.values.head)).flatMap(
    _.set(OtherOfficialAddressLookupPage(index), ConfirmedAddressConstants.address)).success.value
  ...
  "OtherOfficialsStatusHelper" must {
    ...
    "when verifying section 8 answers" must {
      ...
      "return false when only one other official is provided" in {
        ...
        helper.checkComplete(common(0, emptyUserAnswers)
        .set(IsOtherOfficialNinoPage(0), true)
        .flatMap(_.set(OtherOfficialsNinoPage(0), "AA123123A"))
        .flatMap(_.set(IsOtherOfficialsPreviousAddressPage(0), false)).success.value) mustBe false
      }
      ...
      "return false with two other officials with additional data for Nino and no previous address (Scenario 2)" in {
        helper.checkComplete(completeFirstTwo
          .addPassport(0).addPassport(1)
          .set(OtherOfficialsNinoPage(0), "AA123123A").success.value
        ) mustBe false
      }
      ...
      "return false with three other officials with no required data for Nino, passport and no previous address (Scenario 2)" in {
        helper.checkComplete(completeAllThree.removeNino(1).addPassport(1)
          .remove(IsOtherOfficialsPreviousAddressPage(0))
          .flatMap(_.remove(IsOtherOfficialsPreviousAddressPage(1))).success.value) mustBe false
      }

      "return true with two other officials with Nino and previous address (Scenario 3)" in {
        helper.checkComplete(completeFirstTwo
          .addAddress(0).addAddress(1)) mustBe true

      }

      "return false with two other officials with no required data for Nino and previous address (Scenario 3)" in {
        helper.checkComplete(completeFirstTwo
          .removeNino(0).removeNino(1).addAddress(0).addAddress(1)) mustBe false
      }

      "return false with two other officials with additional data for Nino and previous address (Scenario 3)" in {
        helper.checkComplete(completeFirstTwo
          .addPassport(0).addPassport(1)
          .addAddress(0).addAddress(1)
          .set(OtherOfficialsNinoPage(0), "AA123123A")
          .flatMap(_.set(OtherOfficialsNinoPage(1), "AA123123A")).success.value
        ) mustBe false
      }

      "return true with three other officials with Nino and previous address (Scenario 3)" in {
        helper.checkComplete(completeAllThree
          .addAddress(0).addAddress(1).addAddress(2)) mustBe true
      }

      "return true with three other officials with Nino, passport and previous address (Scenario 3)" in {
        helper.checkComplete(completeAllThree
          .addPassport(1).removeNino(1).addPassport(2).removeNino(2)
          .addAddress(0).addAddress(1)) mustBe true
      }

      "return false with three other officials with no required data for Nino, passport and previous address (Scenario 3)" in {
        helper.checkComplete(completeAllThree.addPassport(1).addPassport(2).removeNino(1).removeNino(2)
          .remove(IsOtherOfficialsPreviousAddressPage(0)).flatMap(
          _.remove(IsOtherOfficialsPreviousAddressPage(1))).flatMap(
          _.remove(IsOtherOfficialsPreviousAddressPage(2))).success.value) mustBe false
      }

      "return true with two other officials with passport and no previous address (Scenario 4)" in {
        helper.checkComplete(completeFirstTwo
          .addPassport(0).addPassport(1).removeNino(0).removeNino(1)) mustBe true
      }

      "return false with two other officials with no required data for passport and no previous address (Scenario 4)" in {
        helper.checkComplete(completeFirstTwo.removeNino(0).removeNino(1)
          .set(IsOtherOfficialNinoPage(0), false).flatMap(
          _.set(IsOtherOfficialNinoPage(1), false)).success.value) mustBe false
      }
      ...
      "return true with three other officials with passport and previous address (Scenario 5)" in {
        helper.checkComplete(completeAllThree
          .addPassport(0).removeNino(0).addAddress(0)
          .addPassport(1).removeNino(1).addAddress(1)
          .addPassport(2).removeNino(2).addAddress(2)
        ) mustBe true
      }
      ...
      "return false with incorrect data for two other officials" in {
        helper.checkComplete(completeFirstTwo
          .removeNino(0)
          .addPassport(1)
          .remove(OtherOfficialsPassportPage(1)).success.value
        ) mustBe false
      }
      ...
      "return false with incorrect data for three other officials" in {
        helper.checkComplete(completeAllThree
          .addPassport(0).removeNino(0).addAddress(0)
          .addAddress(1).addAddress(2)
          .remove(IsOtherOfficialsPreviousAddressPage(1))
          .flatMap(_.remove(IsOtherOfficialsPreviousAddressPage(2))).success.value
        ) mustBe false
      }
