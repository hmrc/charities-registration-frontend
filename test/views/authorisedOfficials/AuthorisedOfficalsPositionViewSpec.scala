...
class AuthorisedOfficalsPositionViewSpec extends ViewBehaviours  {
  ...
  val form: Form[AuthorisedOfficialsPosition] = inject[AuthorisedOfficialsPositionFormProvider].apply()

    "AuthorisedOfficialsPositionView" must {

      def applyView(form: Form[AuthorisedOfficialsPosition]): HtmlFormat.Appendable = {
          val view = viewFor[AuthorisedOfficialsPositionView](Some(emptyUserAnswers))
      ...
      AuthorisedOfficialsPosition.options(form).zipWithIndex.foreach { case (option, i) =>
