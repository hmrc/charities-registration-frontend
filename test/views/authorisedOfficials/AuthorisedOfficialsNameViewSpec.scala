...
import models.{AuthorisedOfficialsName, Index, NormalMode}
import play.api.data.Form
import play.twirl.api.HtmlFormat
import views.behaviours.QuestionViewBehaviours
import views.html.authorisedOfficials.AuthorisedOfficialsNameView

class AuthorisedOfficialsNameViewSpec extends QuestionViewBehaviours[AuthorisedOfficialsName]  {

  private val messageKeyPrefix = "authorisedOfficialsName"
  override val form: Form[AuthorisedOfficialsName] = inject[AuthorisedOfficialsNameFormProvider].apply()

    "AuthorisedOfficialsNameView" must {

      def applyView(form: Form[_]): HtmlFormat.Appendable = {
          val view = viewFor[AuthorisedOfficialsNameView](Some(emptyUserAnswers))
          view.apply(form, NormalMode, Index(0))(fakeRequest, messages, frontendAppConfig)
        }
